...
for i in range(len(train_list_simple)):
    ...
    save_model(fitted, 'b2_0','ARIMA', 'simple', str(i+1), str(order))

# Rolling window
for i in range(len(train_list_rolling)):
    fitted, order = fit_arima(train_list_rolling[i])
    save_model(fitted, 'b2_0','ARIMA', 'rolling', str(i+1), str(order))

# Expanding window
for i in range(len(train_list_expanding)):
    fitted, order = fit_arima(train_list_expanding[i])
    save_model(fitted, 'b2_0','ARIMA', 'expanding', str(i+1), str(order))

#%% Load folder
arima_folder_path = 'models/b2_0/ARIMA'

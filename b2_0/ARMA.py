...
for i in range(len(train_list_simple)):
    ...
    save_model(fitted, 'b2_0','ARMA', 'simple', str(i+1), '')

#%%
# Rolling window
for i in range(len(train_list_rolling)):
    fitted = fit_arma(train_list_rolling[i], p, 1, q)
    save_model(fitted, 'b2_0','ARMA', 'rolling', str(i+1), '')

# Expanding window
for i in range(len(train_list_expanding)):
    fitted = fit_arma(train_list_expanding[i], p, 1, q)
    save_model(fitted, 'b2_0','ARMA', 'expanding', str(i+1), '')

#%% Load folder
arma_folder_path = 'models/b2_0/ARMA'

#%% Evaluate with simple split
df_evaluation = arma_evaluation(arma_folder_path, 'simple',train_list_simple,val_list_simple,test_data, df_close,
                                 df_evaluation)

#%% Evaluate with rolling window
df_evaluation = arma_evaluation(arma_folder_path, 'rolling',train_list_rolling,val_list_rolling,test_data, df_close,
                                 df_evaluation)

#%% Evaluate with expanding window
df_evaluation = arma_evaluation(arma_folder_path, 'expanding',train_list_expanding,val_list_expanding,test_data,
                                df_close,
                                 df_evaluation)

...
for i in range(len(train_list_simple)):
    fitted = fit_holt_winters(train_list_simple[i], 12, 'mul', 'mul')
    save_model(fitted, 'b2_0','Holt-Winters', 'simple', str(i+1), '')

#%%
# Rolling window
for i in range(len(train_list_rolling)):
    fitted = fit_holt_winters(train_list_rolling[i], 12, 'mul', 'mul')
    save_model(fitted, 'b2_0','Holt-Winters', 'rolling', str(i+1), '')

# Expanding window
for i in range(len(train_list_expanding)):
    fitted = fit_holt_winters(train_list_expanding[i], 12, 'mul', 'mul')
    save_model(fitted, 'b2_0','Holt-Winters', 'expanding', str(i+1), '')

#%% Load folder
holt_folder_path = 'models/b2_0/Holt-Winters'

#%% Evaluate with simple split
df_evaluation = holt_winters_evaluation(holt_folder_path, 'simple',train_list_simple,val_list_simple,
                                  test_data,
                           df_close,
                                 df_evaluation)

#%% Evaluate with rolling window
df_evaluation = holt_winters_evaluation(holt_folder_path, 'rolling',train_list_rolling,val_list_rolling,test_data,
                           df_close,
                                 df_evaluation)

#%% Evaluate with expanding window
df_evaluation = holt_winters_evaluation(holt_folder_path, 'expanding',train_list_expanding,val_list_expanding,
                           test_data, df_close,
                                 df_evaluation)

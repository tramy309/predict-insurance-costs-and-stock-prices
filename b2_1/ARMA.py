...
#%% Best p,q
p = find_optimal_p(diff, 10)
q = find_optimal_q(diff, p, 10)
...
for i in range(len(train_list_simple)):
    fitted = fit_arma(train_list_simple[i], p, 1, q)
    save_model(fitted, 'b2_1','ARMA', 'simple', str(i+1), '')

# Rolling window
for i in range(len(train_list_rolling)):
    fitted = fit_arma(train_list_rolling[i], p, 1, q)
    save_model(fitted, 'b2_1','ARMA', 'rolling', str(i+1), '')

# Expanding window
for i in range(len(train_list_expanding)):
    fitted = fit_arma(train_list_expanding[i], p, 1, q)
    save_model(fitted, 'b2_1','ARMA', 'expanding', str(i+1), '')

#%% Load folder
ar_folder_path = 'models/b2_1/ARMA'

#%% Evaluate with simple split
df_evaluation = arma_evaluation(ar_folder_path,'simple',train_list_simple,val_list_simple,test_data, df_close,
                                 df_evaluation)

#%% Evaluate with rolling window
df_evaluation = arma_evaluation(ar_folder_path,'rolling',train_list_rolling,val_list_rolling,test_data, df_close,
                                 df_evaluation)

#%% Evaluate with expanding window
df_evaluation = arma_evaluation(ar_folder_path,'expanding',train_list_expanding,val_list_expanding,test_data,
                                df_close,
                                 df_evaluation)

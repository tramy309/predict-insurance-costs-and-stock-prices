...
for i in range(len(train_list_simple)):
    fitted = fit_holt_winters(train_list_simple[i], 12, 'add', 'add')
    save_model(fitted, 'b2_1','Holt-Winters', 'simple', str(i+1), '')

# Rolling window
for i in range(len(train_list_rolling)):
    fitted = fit_holt_winters(train_list_rolling[i], 12, 'add', 'add')
    save_model(fitted, 'b2_1','Holt-Winters', 'rolling', str(i+1), '')

# Expanding window
for i in range(len(train_list_expanding)):
    fitted = fit_holt_winters(train_list_expanding[i], 12, 'add', 'add')
    save_model(fitted, 'b2_1','Holt-Winters', 'expanding', str(i+1), '')

#%% Load folder
holt_folder_path = 'models/b2_1/Holt-Winters'
